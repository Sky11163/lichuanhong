...
const renderOption = (item: DataSourceType) => {
    const itemWithNumber = item as DataSourceType<testTemplateProps>;
    return(
        <div className="item">
...
describe('test AutoComplete component', () => {
    ...
    it('test basic AutoComplete behavior', async() => {
        ...
        expect(testProps.onSelect).toHaveBeenLastCalledWith({value: 'ab', number: 11 });
    ...
    it('async fetchSuggestions should works fine', async() => {
        cleanup();
        wrapper = render(<AutoComplete {...testAsyncProps} />);
        inputNode = wrapper.getByPlaceholderText('auto-complete') as HTMLInputElement;
        fireEvent.change(inputNode, {target: { value: 'a'}})
        await wait(() => {
            expect(testAsyncProps.fetchSuggestions).toHaveBeenCalled()
            expect(wrapper.queryByText('ab')).toBeInTheDocument()
        })
    })
})